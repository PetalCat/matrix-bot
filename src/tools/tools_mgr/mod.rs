...
pub fn register_defaults(specs: &mut Vec<ToolSpec>) {
    if !specs.iter().any(|t| t.id == "tools") {
        specs.push(ToolSpec {
            ...
            enabled: true,
            dev_only: None,
            triggers: ToolTriggers { commands: vec!["!tools".into()], mentions: vec![] },
            config: serde_yaml::Value::default(),
        });
    }
}

pub fn build() -> Arc<dyn Tool> { Arc::new(ToolsManager) }

pub struct ToolsManager;

#[async_trait]
impl Tool for ToolsManager {
    fn id(&self) -> &'static str { "tools" }
    fn help(&self) -> &'static str { "Manage tools: !tools list | enable <id> | disable <id>" }
    async fn run(&self, ctx: &ToolContext, args: &str, _spec: &ToolSpec) -> Result<()> {
        let registry = &ctx.registry;
        let mut parts = args.split_whitespace();
        match parts.next() {
            Some("list") | None => {
                let mut rows = vec!["tools:".to_string()];
                for (id, entry) in registry.by_id.iter() {
                    ...
                    let triggers = format!("cmds=[{}], mentions=[{}]",
                        entry.spec.triggers.commands.join(", "),
                        entry.spec.triggers.mentions.join(", "));
                    rows.push(format!("- {}: enabled={} dev_only={} {}", id, enabled, dev_only, triggers));
                }
            ...
            Some("enable") => {
                if let Some(id) = parts.next() {
                    ...
                    st.insert(id.to_string(), true);
                    send_text(ctx, format!("enabled tool: {}", id)).await
                } else { send_text(ctx, "Usage: !tools enable <id>").await }
            }
            Some("disable") => {
                if let Some(id) = parts.next() {
                    let mut st = registry.state.lock().await;
                    st.insert(id.to_string(), false);
                    send_text(ctx, format!("disabled tool: {}", id)).await
                } else { send_text(ctx, "Usage: !tools disable <id>").await }
            }
